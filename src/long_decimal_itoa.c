...
/*   Updated: 2017/08/30 14:20:10 by aserguie         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static int		get_decimal_len(long n)
{
	int len;
...
static void		fill_number(char *buff, long n, int start, int len)
{
	if (n < 0)
	{
		buff[start--] = -(n % 10) + '0';
...
static void		fill_flags(char *buff, long n, t_buff_nfo bi, t_parse data)
{
	int	pos_sign;
